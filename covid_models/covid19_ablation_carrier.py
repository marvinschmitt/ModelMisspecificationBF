...
def non_stationary_SEICR(params_sir, params_secir, N, T, sim_diff=16, observation_model=True):
    ...
    beta, gamma, delta, d = params_secir
    ...
    for t in range(T+sim_lag):
        ...
        E_new = lambd_arr[t] * ((beta*I[t])/N)*S[t]
        ...
        # Calculate current infected by adding the exposed people who become infected
        # subtracting the dead and recovered. The newly infected are jsut the exposed
        # and the model assumes that everybody is detected
        I_t = I[t] + gamma*E[t] - (1-delta)*mu*I[t] - delta*d*I[t]
        I_new = gamma*E[t]
